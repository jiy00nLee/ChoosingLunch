...
class PlaceKoreanService {
    ...
    fun getAll(query: String, userLng : Double, userLat: Double) : List<PlaceDocument>{
        val listPlaces : List<PlaceKorean> = placeRepository.findAll()
        println(listPlaces)

        //리스트를 통째로 넘겨준 후 거리계산해서 해당되는 애들만 거리순으로 정리해 리스트 추출하기.
        val selectedlist : MutableList<PlaceKorean> = mutableListOf()
    ...
    private fun calculateDistance(x: Double, y : Double, place : PlaceKorean) : Boolean {
