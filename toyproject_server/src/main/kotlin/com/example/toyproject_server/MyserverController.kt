...
class MyserverController {
    ...
    fun getFromKakao(
        ...
        val final_storelist_idnames = Service(userLng, userLat, query) //return hashmap.
        menuService.makeMenus(final_storelist_idnames)

    }


    private fun Service(userLng: Double, userLat: Double, query: String):  HashMap<String, String> {
        val querylist: MutableList<String> = mutableListOf()
        val storelist_idnames:  HashMap<String, String> = hashMapOf()
        ...
        if (query == "한식") {
            ...
            querylist.add("파스타")
            querylist.add("레스토랑")
        } else return storelist_idnames  //query == "음식점"일 경우 저장하지않는다.

        for (i in querylist) {
            placeService.saveAll(i, userLng, userLat, query)?.let {
                storelist_idnames.putAll(it) }
            }
        return storelist_idnames
    }
        ...
        fun getPlaceDB(
            @RequestParam("query") query: String,
            @RequestParam("x") userLng: Double, @RequestParam("y") userLat: Double,
            @RequestParam("userAddress") userAddress: String
        ): List<PlaceDocument> {

            val foundquery: QueryData? =
                queryService.findQuery(query, userLng, userLat, userAddress) //query 찾은 적 있었는지 여부 찾기.
