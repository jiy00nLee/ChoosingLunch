...
@Transactional
class PlaceChineseService {
    private val placeRepository : PlaceChineseRepository

    @Autowired
    constructor(placeRepository : PlaceChineseRepository){
    ...
    fun saveAll(query: String?, userLng : Double, userLat: Double)  {
        ...
        var transed_places : List<PlaceChinese>?
    ...
    fun getAll(query: String, userLng : Double, userLat: Double) : List<PlaceDocument>{
        val listPlaces : List<PlaceChinese> = placeRepository.findAll()
        println(listPlaces)

        //리스트를 통째로 넘겨준 후 거리계산해서 해당되는 애들만 거리순으로 정리해 리스트 추출하기.
        val selectedlist : MutableList<PlaceChinese> = mutableListOf()
    ...
    private fun calculateDistance(x: Double, y : Double, place :PlaceChinese) : Boolean {
    ...
    private fun mappingPlaceDocumenttoPlace(placeDocument: PlaceDocument) : PlaceChinese {
        val place : PlaceChinese = PlaceChinese()
    ...
    private fun mappingPlacetoPlaceDocument(place: PlaceChinese) : PlaceDocument {
